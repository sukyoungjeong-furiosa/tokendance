...
└─ supervisor.py  (nohup으로 띄운 유일한 상주 프로세스)
      ...
      └─ 마스터 1회 기동 (동기 실행, 가볍게 끝남)
           ...
           ├─ 5. queued 일감을 상한(4) 내에서 워커로 디스패치 (detached, 안 기다림)
           ├─ 6. 지식 수확물 library/ 반영
           ├─ 7. reports/ 작성 + Slack 요약 푸시
           └─ 종료
                │
                └─ worker = setsid claude -p --append-system-prompt prompts/worker.md
                                          --dangerously-skip-permissions  &
...
tokendance/
  ...
  scripts/
    ...
    launch-worker.sh              # worktree 셋업 + setsid claude 워커 기동
    prepare-worktree.sh           # 공통 artifact 셋업 (1차는 stub, dogfood로 정교화)
    start.sh / stop.sh            # supervisor 기동/정지
  state/
    inbox.md                      # 모든 사람 입력이 수렴하는 곳
    tasks/<task-id>/
      task.md                     # 일감 스펙, 출처, 완료 기준
      status.json                 # 상태 머신 (§6.1)
      progress.md                 # 워커의 현재 진행 서술 (peek 대상)
      steer.md                    # 마스터/사람의 중간 지시 (steer 입력)
      log.md                      # 진행/결정 로그 (지식 C)
      review.md                   # 마스터의 리뷰 노트
...
```
- `heartbeat`는 워커가 체크포인트마다 갱신. 마스터는 신선도로 생사를 판단한다.
- `worker_pid`로 마스터/supervisor가 실제 프로세스 생존을 교차 확인한다.
- `worker_session_id`로 반려된 일감을 `--resume`해 컨텍스트를 이어 재투입한다.

### 6.2 마스터의 status 분기
- `running` & heartbeat 신선 & pid 살아있음 → **냅둠**. progress.md 훑어 헤매면 steer.md 기록.
- `running` & (heartbeat 멈춤 또는 pid 죽음) → **죽은 워커**. log/progress 보고 재투입 또는 사람에게 에스컬레이션.
- `review` → **마스터 직접 리뷰**(§7). done 또는 반려.
- `blocked` → 막힌 이유를 리포트 🔴에 올림. 사람 입력 대기.
- `queued` & 동시 실행 < 4 → **워커 디스패치**.
...
2. `steer.md`를 읽고, 지난 확인 이후 새 내용이 있으면 반영하고 반영 사실을 log.md에 남긴다.
3. `status.json`의 heartbeat 갱신.
4. 계속 진행.

### 마스터/사람의 주입:
- 마스터는 헤매는 워커의 `steer.md`에 교정 한 줄.
- 사람은 inbox.md 또는 직접 steer.md에 한 줄 → 다음 체크포인트에 반영.

## 9. 입력 채널 → inbox 수렴

터미널 직접 편집 / Slack / remote-control 에이전트 — 모두 종착지는 `state/inbox.md`. 마스터는 출처를 몰라도 된다. Slack 어댑터는 깨어남마다 지정 채널/DM을 읽어 새 지시를 inbox로 옮긴다.

## 10. 리포트 (진행+의심 중심)

`reports/YYYY-MM-DD.md`에 누적, Slack에 요약 푸시:
- 🟢 **순항**: (일감, 한 줄 현황)
- 🟡 **판단 필요**: "X가 애매해서 일단 Y로 진행 중 — 맞나요?" → 답이 steer로 흘러감
- 🔴 **막힘**: 이유 + 마스터 제안
- ✅ **리뷰 완료**: 검수 결과물 + 한 줄 평
...
### MVP (손으로 구축)
- state/ 디렉토리 구조 + status.json 상태 머신
- supervisor.py (30분 타이머, 마스터 기동, 헬스체크)
- launch-worker.sh (setsid 워커 기동, pid/로그 캡처)
- prompts/master.md, prompts/worker.md (peek/steer 프로토콜 포함)
- 리뷰 루프, 진행+의심 리포트
- Slack 입력 어댑터 + 출력 푸시
