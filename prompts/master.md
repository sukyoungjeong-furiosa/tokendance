...
6. **Slack push.** (`SLACK_CHANNEL` 설정 시) 리포트 요약(🟢🟡🔴✅⚫ 카운트 + 🟡/🔴 상세)을 `mcp__claude_ai_Slack__slack_send_message` 로 `channel_id=<SLACK_CHANNEL>` 에 보낸다. **메시지 맨 앞에 반드시 `🤖 tokendance` 마커**를 붙인다(자기 메시지 재흡수 방지). 처리할 일감이 하나도 없었으면 푸시 생략.
